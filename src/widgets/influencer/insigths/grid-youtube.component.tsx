...
import {
  useYoutubeLocalStatisticsByInfluencerIdQuery,
  useYoutubeLocalStatisticsLogsByInfluencerIdQuery,
  useYoutubePostsStatisticsByInfluencerIdQuery,
...
export const InfluencerInsightsGridYoutube = (props: InfluencerInsightsGridProps) => {
  ...
  } = useYoutubeLocalStatisticsByInfluencerIdQuery(influencer.id);
  ...
  return (
    <div className="w-9/12 pl-8">
      ...
      <div className="mb-3 grid grid-cols-4 gap-3">
        <StatisticsItem
          label="Followers"
          value={localStatistics.subscribersCount}
          previousValue={localStatisticsLogs?.[0]?.subscribersCount}
        />
        <StatisticsItem
          label="Average Views"
          value={localStatistics.views}
          previousValue={localStatisticsLogs?.[0]?.views}
          tooltipText="Average amount of views across all posts"
        />
        <StatisticsItem
          label="Eng. Rate"
          value={`${roundNumberTo2(Number(localStatistics.engagementRate))}%`}
          previousValue={localStatisticsLogs?.[0]?.engagementRate}
          tooltipText="Ratio of an app's engaged users and active users"
        />
        <StatisticsItem
          label="Completion Rate"
          value={`${roundNumberTo2(
            Number(localStatistics.rate?.averageViewPercentage || 0)
          )}%`}
          previousValue={localStatisticsLogs?.[0]?.rate?.averageViewPercentage}
      ...
      <div className="flex mt-3 space-x-3 justify-start">
        <div className="w-6/12 p-4 bg-white rounded-xl shadow-2xl flex flex-col justify-start">
