import { useTikTokStatisticsByInfluencerIdWithVerificationCodeQuery } from "@entities/tik-tok-statistics";
import { useTikTokPostsStatisticsByInfluencerIdWithVerificationCodeQuery } from "@entities/tik-tok-statistics/model/queries/use-tik-tok-posts-statistics-by-influencer-id-with-verification-code-query.hook";
...
type InfluencerTikTokType = [influencer: IInfluencer, shareToken: string];

export const ShareInfluencerTikTokInsightsPage = () => {
  ...
  } = useTikTokStatisticsByInfluencerIdWithVerificationCodeQuery({
  ...
  } = useTikTokPostsStatisticsByInfluencerIdWithVerificationCodeQuery({
    influencerId: influencer.id,
    verificationCode: shareToken,
    from: store.dates.from,
    to: store.dates.to,
  ...
  return (
    <div className="w-9/12 pl-8">
      ...
      <div className="mb-3 grid grid-cols-4 gap-3">
        ...
        />
        <StatisticsItem label="Likes" value={Number(localStatistics.likesCount)} />
        <StatisticsItem label="Videos" value={Number(localStatistics.videosCount)} />
      </div>
                    ...
                    <DoughnutDiagram
                      data={{
                        ...
                        datasets: [
                          {
                            label: " ",
                            data: countriesStatistics.map((country) =>
                              Number((Number(country.count) * 100).toFixed(2))
                            ),
                  ...
                  <div className="space-y-3 mt-auto">
                    {countriesStatistics.map((country, index) => (
                      <InfluencerDiagramLegendElem
                        ...
                        value={Number((Number(country.count) * 100).toFixed(2))}
                      />
                    ))}
                  </div>
                </>
