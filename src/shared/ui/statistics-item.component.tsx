...
import { kFormatter, roundNumberTo2 } from "@shared/lib/utils";
...
export const StatisticsItem = (props: IStatisticsItemProps) => {
  const { label, value, previousValue, tooltipText } = props;

  const calculateDifferencePercents = (): number => {
    let currentValue = value;
    if (typeof value === "string") {
      currentValue = value.split("%")[0] || 0;
    }
    ...
    if (
      ...
      Number.isNaN(previousValueNumber) ||
    ...
    return ((currentValueNumber - previousValueNumber) / previousValueNumber) * 100;
  };

  const differencePercents = calculateDifferencePercents();
    ...
    <div className="p-2 rounded-xl shadow-2xl bg-white">
      ...
      <div>
        <p className="text-3xl">
          {typeof value === "number" ? kFormatter(value) : value}
        </p>
        {differencePercents !== 0 && (
          <p
            className={clsx(
              "text-xl",
              differencePercents > 0 ? "text-green" : "text-red-600"
            )}
          >
            {differencePercents > 0 ? "+" : ""}
            {roundNumberTo2(differencePercents)}%
          </p>
        )}
      </div>
    </div>
